    ...
    <div style={{ padding:"20px 16px", display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        {[
          ...
          { label:"語彙を覚える",  icon:"🔤", tab:"vocab",   color:C.accent,   bg:C.accentLight },
        ].map(a => (
...
function PhrasesTab({ phrases, setPhrases }) {
  ...
  // FIX: docx文字化け修正 - mammothのarrayBuffer処理を改善
      ...
      if (file.name.endsWith(".docx")) {
        ...
        r.onload = async ev => {
          try {
            ...
            // mammothのオプションでスタイル変換を無効化してテキストのみ取得
        ...
        r.onload = ev => {
          ...
          if (mojibakeCount > 5) {
            ...
            r2.onerror = () => resolve({ name: file.name, text });
  ...
  async function doImport() {
    ...
    for (let i = 0; i < sources.length; i++) {
      ...
      try {
        const sys = `You are an English learning assistant for a Japanese pharmaceutical regulatory affairs professional.
Assign difficulty levels to English phrases using these STRICT criteria:
- 初級: Short phrases, common everyday words, junior high school English level, simple greetings or basic business expressions (e.g. "Thank you for your email.")
- 中級: Contains multiple clauses, business-specific vocabulary, moderately technical terms, compound sentences (e.g. "I appreciate your time and consideration.")  
- 上級: Long complex sentences, regulatory/pharmaceutical/legal terminology, complex grammar structures, difficult conjunctions (e.g. "If the following conditions are met, we would like to visit the regulatory authority...")
...
Return ONLY valid JSON array with no other text: [{"english":"...","level":"初級"|"中級"|"上級"}]`;
        ...
        const leveled = JSON.parse(jsonMatch[0]);
        raw.forEach(p => {
  ...
  async function addPhraseWithLevel() {
    ...
    try {
      const sys = `You are evaluating difficulty for a Japanese pharmaceutical professional learning English.
Return ONLY one word — 初級, 中級, or 上級 — using these criteria:
- 初級: Short, simple, common words only, basic business greetings
- 中級: Multiple clauses, business vocabulary, moderately technical
- 上級: Long/complex sentences, regulatory/medical/legal terms, complex grammar`;
      const resp = (await callClaude(sys, newP.english)).trim();
      if (LEVELS.includes(resp)) level = resp;
    } catch {}
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
        ...
        <div style={{ display:"flex", gap:5, overflowX:"auto", paddingBottom:6 }}>
          {["すべて",...LEVELS].map(l => (
            <button key={l} onClick={() => setLv(l)} style={{
              ...
              background: lv===l ? (l==="すべて"?C.slate:levelColor(l)) : C.surface,
              color: lv===l ? "#fff" : C.muted,
            }}>{l}</button>
          ))}
        </div>
            ...
            {expanded===p.id && (
              <div style={{ padding:"0 13px 11px", borderTop:`1px solid ${C.surface}` }}>
                ...
                <div style={{ display:"flex", gap:8, marginTop:8, alignItems:"center" }}>
                  ...
                  <button onClick={() => setPhrases(prev=>prev.filter(x=>x.id!==p.id))} style={{ background:"none", border:`1px solid ${C.dangerMid}`, color:C.danger, borderRadius:6, padding:"2px 8px", fontSize:10, cursor:"pointer" }}>削除</button>
                </div>
              </div>
            )}
          </div>
      ...
      {showAdd && (
        <Modal onClose={() => setShowAdd(false)}>
          ...
          <Field label="難易度（AIが自動判定、変更可）">
            <div style={{ display:"flex", gap:6 }}>
              {LEVELS.map(l=>(
                <button key={l} onClick={()=>setNewP(p=>({...p,level:l}))} style={{ flex:1, padding:"6px 0", borderRadius:8, border:"none", cursor:"pointer", fontSize:12, fontWeight:700, background:newP.level===l?levelColor(l):C.surface, color:newP.level===l?"#fff":levelColor(l) }}>{l}</button>
              ))}
            </div>
          </Field>
      ...
      {showImport && (
        <Modal onClose={() => { setShowImport(false); setImportText(""); setImportResult(null); setImportFiles([]); setImportProgress(""); }}>
          ...
          <p style={{ fontSize:11, color:C.muted, margin:"0 0 12px" }}>複数ファイルをまとめて選択、またはテキストを貼り付けてください。AIが英語フレーズを抽出・難易度判定します。</p>

          {!importResult ? (
            <>
              <label style={{
                display:"flex", alignItems:"center", gap:8, padding:"12px 14px",
                border:`2px dashed ${C.primaryMid}`, borderRadius:12, cursor:"pointer",
                background:C.primaryLight, marginBottom:10,
              }}>
                <span style={{ fontSize:20 }}>📂</span>
                <div>
                  <div style={{ fontSize:13, fontWeight:700, color:C.primary }}>ファイルを選択（複数可）</div>
                  <div style={{ fontSize:11, color:C.muted }}>.docx / .txt に対応</div>
                </div>
                <input type="file" accept=".txt,.docx" multiple onChange={handleMultiFileSelect} style={{ display:"none" }} />
              </label>

              {importFiles.length > 0 && (
                <div style={{ background:C.successLight, border:`1px solid ${C.successMid}`, borderRadius:10, padding:10, marginBottom:10 }}>
                  <div style={{ fontSize:11, fontWeight:700, color:C.success, marginBottom:6 }}>✅ {importFiles.length}ファイル選択済み</div>
                  {importFiles.map((f,i) => (
                    <div key={i} style={{ fontSize:11, color:C.mid, marginBottom:2 }}>📄 {f.name}</div>
                  ))}
                </div>
              )}
              ...
              {importProgress && (
                <div style={{ fontSize:12, color:C.primary, marginTop:8, textAlign:"center" }}>⏳ {importProgress}</div>
              )}
            ...
            <>
              <div style={{ background:C.successLight, border:`1px solid ${C.successMid}`, borderRadius:10, padding:10, marginBottom:10 }}>
                <div style={{ fontSize:13, fontWeight:700, color:C.success }}>✅ {importResult.length}件抽出しました（重複除去済み）</div>
                {importFiles.length>0 && <div style={{ fontSize:11, color:C.muted, marginTop:2 }}>{importFiles.length}ファイルから抽出</div>}
              </div>
                ...
                {importResult.map(p=>(
                  <div key={p.id} style={{ padding:"6px 0", borderBottom:`1px solid ${C.surface}`, fontSize:12 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:2 }}>
                      ...
                      <span style={{ fontSize:9, color:C.subtle }}>{p.source}</span>
                    </div>
                    <span style={{ fontWeight:600 }}>{p.english}</span>
                    {p.japanese && <div style={{ color:C.muted, fontSize:11 }}>{p.japanese}</div>}
                  </div>
                ))}
              </div>
              <ModalButtons
...
function VocabTab({ vocab, setVocab }) {
  ...
  async function addVocab() {
    ...
    try {
      const sys = `Return ONLY one of: 初級, 中級, 上級 — difficulty of this English word/idiom for a Japanese pharmaceutical professional.
- 初級: common everyday words
- 中級: business/moderately technical
- 上級: regulatory/medical/legal/rare`;
      const resp = (await callClaude(sys, newV.word)).trim();
      if (LEVELS.includes(resp)) level = resp;
    } catch {}
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      <div style={{ padding:"14px 16px 0" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
          ...
          <div style={{ display:"flex", gap:6 }}>
            <button onClick={()=>setMode(mode==="list"?"flash":"list")} style={{ background:C.surface, border:"none", borderRadius:8, padding:"5px 8px", fontSize:11, cursor:"pointer", color:C.mid, fontWeight:600 }}>
              {mode==="list"?"🃏 カード":"📋 リスト"}
            </button>
            <button onClick={()=>setShowAdd(true)} style={{ background:C.primary, border:"none", borderRadius:8, padding:"5px 8px", fontSize:11, cursor:"pointer", color:"#fff", fontWeight:600 }}>＋追加</button>
          </div>
        ...
        <div style={{ display:"flex", gap:5, overflowX:"auto", paddingBottom:6 }}>
          {["すべて",...LEVELS].map(l=>(
            <button key={l} onClick={()=>setLv(l)} style={{ padding:"3px 10px", borderRadius:99, border:"none", cursor:"pointer", whiteSpace:"nowrap", fontSize:11, fontWeight:600, background:lv===l?(l==="すべて"?C.slate:levelColor(l)):C.surface, color:lv===l?"#fff":C.muted }}>{l}</button>
          ))}
        </div>
        <div style={{ display:"flex", gap:5, overflowX:"auto", paddingBottom:8, marginTop:4 }}>
          {VOCAB_CATS.map(c=>(
            <button key={c} onClick={()=>setCat(c)} style={{ padding:"3px 10px", borderRadius:99, border:"none", cursor:"pointer", whiteSpace:"nowrap", fontSize:11, fontWeight:600, background:cat===c?C.accent:C.surface, color:cat===c?"#fff":C.muted }}>{c}</button>
          ))}
        </div>
      </div>
          ...
          {filtered.length===0 ? <div style={{ color:C.subtle, marginTop:40 }}>語彙がありません</div> : (
            <>
              ...
              <div onClick={()=>setFlipped(v=>!v)} style={{
                width:"100%", minHeight:200, background:flipped?C.slate:C.card,
                border:`2px solid ${C.border}`, borderRadius:20, padding:24,
                ...
                {!flipped ? (
                  ...
                  <>
                    ...
                    {card.example && <div style={{ fontSize:12, color:C.subtle, lineHeight:1.5, fontStyle:"italic" }}>{card.example}</div>}
                  </>
                )}
              </div>
...
function QuizTab({ phrases, vocab, setProgress, weaknesses, setWeaknesses }) {
  ...
  async function checkTyping() {
    ...
    setChecking(true);
    try {
      const sys = `You are an English teacher. Compare the student's answer to the correct answer and respond ONLY in JSON with no extra text:
...
Be lenient with minor punctuation differences but strict with spelling and grammar.`;
      ...
      <Field label="出題元">
        <div style={{ display:"flex", gap:8 }}>
          {[["phrases","表現集"],["vocab","語彙"]].map(([v,l])=>(
            <button key={v} onClick={()=>setSource(v)} style={{ flex:1, padding:"8px 0", borderRadius:8, border:`2px solid ${source===v?C.primary:C.border}`, background:source===v?C.primaryLight:C.card, cursor:"pointer", fontSize:13, fontWeight:source===v?700:400, color:source===v?C.primary:C.muted }}>{l}</button>
          ))}
        </div>
      </Field>
      <Field label="レベル">
      ...
      <Field label="回答方法">
        <div style={{ display:"flex", gap:8 }}>
          {[["self","脳内確認"],["typing","タイピング"]].map(([v,l])=>(
            <button key={v} onClick={()=>setInputMode(v)} style={{ flex:1, padding:"8px 0", borderRadius:8, border:`2px solid ${inputMode===v?C.purple:C.border}`, background:inputMode===v?C.purpleLight:C.card, cursor:"pointer", fontSize:13, fontWeight:inputMode===v?700:400, color:inputMode===v?C.purple:C.muted }}>{l}</button>
          ))}
        </div>
      </Field>
      {weaknesses.length>0 && (
        <div style={{ background:C.accentLight, border:`1px solid ${C.accentMid}`, borderRadius:10, padding:10, marginBottom:12, fontSize:12, color:"#9a3412" }}>
  ...
  if (mode==="result") {
    ...
    return (
      <div style={{ padding:"24px 16px", textAlign:"center" }}>
        ...
        {weaknesses.length>0 && <div style={{ fontSize:12, color:"#9a3412", marginBottom:12 }}>苦手表現: {weaknesses.length}件</div>}
  ...
  return (
    <div style={{ padding:"14px 16px" }}>
      ...
      <div style={{ background:C.surface, borderRadius:14, padding:20, minHeight:110, display:"flex", alignItems:"center", justifyContent:"center", textAlign:"center", marginBottom:14, border:`1px solid ${C.border}` }}>
        <div>
      ...
      {inputMode==="typing" && !revealed && (
        <input value={userInput} onChange={e=>setUserInput(e.target.value)} placeholder="ここに入力…" style={{ width:"100%", padding:"10px 12px", borderRadius:10, border:`1px solid ${C.border}`, fontSize:14, boxSizing:"border-box", marginBottom:10, outline:"none" }} />
      )}

      {!revealed ? (
        inputMode==="typing" ? (
          <button onClick={checkTyping} disabled={checking||!userInput.trim()} style={{ width:"100%", padding:14, borderRadius:12, border:"none", background:checking?C.subtle:C.purple, color:"#fff", fontSize:15, fontWeight:700, cursor:"pointer" }}>
        ...
        <div>
          {inputFeedback ? (
            ...
            <div style={{ background:C.successLight, border:`1px solid ${C.successMid}`, borderRadius:12, padding:14, marginBottom:12, textAlign:"center" }}>
              ...
              <div style={{ fontSize:14, fontWeight:700 }}>{quizType==="en2ja"?(p.japanese||"（日本語なし）"):quizType==="fillblank"?<span style={{color:C.success,fontWeight:800}}>{blankWord}</span>:p.english}</div>
            </div>
          )}
          {inputFeedback ? (
            <button onClick={()=>answer(inputFeedback.correct)} style={{ width:"100%", padding:12, borderRadius:10, border:"none", background:C.primary, color:"#fff", fontSize:14, fontWeight:700, cursor:"pointer" }}>次へ</button>
          ) : (
...
function DiaryTab({ setPhrases, weaknesses }) {
  ...
  // FIX: entriesが変わるたびに必ず保存
  useEffect(() => {
    save(STORAGE.diary, entries);
  }, [entries]);
  ...
  async function submitDiary() {
    ...
    setLoading(true);
    setError("");
    setLoadingMsg("AIが添削中… (10〜20秒)");

    try {
      const weakList = weaknesses.map(w=>w.english).join(", ");

      // FIX: JSON生成を確実にするため、プロンプトをより厳密に設定
      const sys = `You are an English teacher for Eriko, a Japanese pharmaceutical regulatory affairs professional.
She writes an English diary to improve her language skills.

Analyze her diary entry and respond with ONLY a valid JSON object (no markdown, no explanation, no text before or after the JSON):

{
  "corrected": "full corrected diary text here",
  "corrections": [
    {"original": "wrong phrase", "corrected": "correct phrase", "explanation": "Japanese explanation"}
  ],
  "patterns": [
    {"pattern": "grammar pattern name", "explanation": "Japanese explanation", "example": "example sentence"}
  ],
  "newPhrases": [
    {"english": "useful phrase", "japanese": "Japanese meaning", "context": "when to use", "level": "初級"}
  ],
  "weakPoints": ["weakness description in Japanese"]
}

Rules:
- corrections: list actual errors found (empty array [] if no errors)
- patterns: 2-3 key grammar patterns to remember
- newPhrases: 3-5 useful phrases from the corrected text
- newPhrases level must be one of: 初級, 中級, 上級
- weakPoints: 1-3 specific weaknesses observed
- Her known weak areas: ${weakList || "none yet"}`;

      const resp = await callClaude(sys, draft);

      // FIX: JSONの抽出を改善 - 複数の抽出パターンを試みる
      let result = null;
      
      // パターン1: そのままパース
      try {
        result = JSON.parse(resp.trim());
      } catch {}

      // パターン2: コードブロックを除去してパース
      if (!result) {
        try {
      ...
      result.newPhrases = result.newPhrases || [];
      result.weakPoints = result.weakPoints || [];
      result.corrected = result.corrected || draft;

      const entry = { id:uid(), date:today(), original:draft, ...result };
      
      // FIX: setEntries → useEffect → save の順で確実に保存
      setEntries(prev => {
        const updated = [entry, ...prev];
        save(STORAGE.diary, updated); // 即座にも保存
        return updated;
      });

      if (result.newPhrases?.length) {
        setPhrases(prev=>[...result.newPhrases.map(p=>({
          ...p,
          id:uid(),
          category:"日記表現",
          source:"日記添削",
      ...
      setError(`添削に失敗しました: ${e.message || "不明なエラー"}。もう一度お試しください。`);
    }
    setLoading(false);
    setLoadingMsg("");
  }
  ...
  if (mode==="write") return (
    <div style={{ padding:"16px", display:"flex", flexDirection:"column", height:"100%" }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
        <button onClick={()=>{ setMode("list"); setError(""); }} style={{ background:"none", border:"none", cursor:"pointer", fontSize:20, color:C.muted }}>←</button>
      ...
      <textarea
        ...
        placeholder="Today I went to... / I had a meeting with... / I felt..."
        style={{ flex:1, padding:14, borderRadius:12, border:`1px solid ${C.border}`, fontSize:14, resize:"none", fontFamily:"inherit", outline:"none", lineHeight:1.7, minHeight:200 }}
      />
      <div style={{ fontSize:11, color:C.subtle, margin:"8px 0", textAlign:"right" }}>{draft.length}文字</div>
      {error && (
        <div style={{ background:C.dangerLight, border:`1px solid ${C.dangerMid}`, borderRadius:10, padding:10, marginBottom:10, fontSize:12, color:C.danger }}>
          ⚠️ {error}
        </div>
      )}
      <button onClick={submitDiary} disabled={loading||!draft.trim()} style={{ padding:14, borderRadius:12, border:"none", background:loading?C.subtle:C.purple, color:"#fff", fontSize:15, fontWeight:700, cursor:loading?"not-allowed":"pointer" }}>
        {loading ? loadingMsg : "✨ 添削してもらう"}
      </button>
    </div>
  );
  ...
  if (mode==="view" && current) return (
    <div style={{ flex:1, overflowY:"auto", padding:"14px 16px" }}>
      ...
      <Section title="✅ 修正後の日記" color={C.success}>
        <div style={{ fontSize:13, lineHeight:1.8, color:C.slate }}>{current.corrected}</div>
      </Section>

      {current.corrections?.length>0 && (
        <Section title={`🔍 修正箇所 (${current.corrections.length}件)`} color={C.danger}>
          {current.corrections.map((c,i)=>(
            <div key={i} style={{ marginBottom:12, paddingBottom:12, borderBottom:i<current.corrections.length-1?`1px solid ${C.surface}`:"none" }}>
      ...
      {current.patterns?.length>0 && (
        <Section title="📌 覚えてほしい文法パターン" color={C.primary}>
          {current.patterns.map((p,i)=>(
            <div key={i} style={{ marginBottom:10 }}>
              <div style={{ fontSize:13, fontWeight:700, color:C.primary }}>{p.pattern}</div>
              <div style={{ fontSize:12, color:C.mid }}>{p.explanation}</div>
              {p.example && <div style={{ fontSize:12, color:C.muted, fontStyle:"italic", marginTop:2 }}>例: {p.example}</div>}
            </div>
          ))}
        </Section>
      )}
      ...
      {current.newPhrases?.length>0 && (
        <Section title={`📚 表現集に追加しました (${current.newPhrases.length}件)`} color={C.purple}>
          {current.newPhrases.map((p,i)=>(
            <div key={i} style={{ marginBottom:6, fontSize:12 }}>
              <span style={{ fontWeight:700 }}>{p.english}</span>
              {p.japanese && <span style={{ color:C.muted }}> — {p.japanese}</span>}
            </div>
          ))}
        </Section>
      )}
      ...
      {current.weakPoints?.length>0 && (
        <Section title="⚠️ 今回の弱点" color={C.warn}>
...
function GoalsTab({ goals, setGoals, progress, weaknesses, phrases, vocab }) {
  ...
  return (
    <div style={{ overflowY:"auto", padding:"16px" }}>
        ...
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:14, marginBottom:14 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            {[
              ...
              { label:"表現集 (CSV)", sub:`${phrases.length}件`, onClick:()=>downloadCSV("phrases"), color:C.success },
              { label:"語彙 (CSV)", sub:`${vocab.length}件`, onClick:()=>downloadCSV("vocab"), color:C.purple },
            ].map(b => (
              <button key={b.label} onClick={b.onClick} style={{ background:C.card, border:`1px solid ${b.color}33`, borderRadius:10, padding:"10px 8px", cursor:"pointer", textAlign:"left" }}>
                <div style={{ fontSize:12, fontWeight:700, color:b.color }}>{b.label}</div>
                <div style={{ fontSize:10, color:C.subtle, marginTop:2 }}>{b.sub}</div>
              </button>
            ))}
          </div>
          <div style={{ fontSize:10, color:C.subtle, marginTop:8 }}>※ CSVはExcelで開けます。JSONは全データの完全バックアップです。</div>
        </div>
      )}
      ...
      <div style={{ background:`linear-gradient(135deg,${C.slate},#334155)`, borderRadius:14, padding:16, marginBottom:14, color:"#fff" }}>
        ...
        <div style={{ display:"flex", gap:16 }}>
          {[{l:"今日の演習",v:todayStudied},{l:"連続日数",v:`${streak}日`},{l:"苦手表現",v:`${weaknesses.length}件`}].map(s=>(
            <div key={s.l}><div style={{ fontSize:20, fontWeight:800 }}>{s.v}</div><div style={{ fontSize:10, color:C.subtle }}>{s.l}</div></div>
          ))}
        </div>
      </div>
          ...
          {last7.map(d=>{
            ...
            return (
              <div key={d} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
                <div style={{ width:"100%", height:Math.max(4,(cnt/max)*50) + "px", background:d===t?C.primary:C.primaryMid, borderRadius:"4px 4px 0 0", transition:"height 0.3s" }} />
                <div style={{ fontSize:8, color:C.subtle }}>{d.slice(5)}</div>
              </div>
            );
          })}
        </div>
      ...
      {weaknesses.length>0 && (
        <div style={{ background:C.accentLight, border:`1px solid ${C.accentMid}`, borderRadius:12, padding:14, marginBottom:14 }}>
          <div style={{ fontSize:11, fontWeight:700, color:"#9a3412", marginBottom:8 }}>⚠️ 苦手表現 TOP{Math.min(5,weaknesses.length)}</div>
          {weaknesses.slice(0,5).map(w=>(
            <div key={w.id} style={{ fontSize:12, color:"#92400e", marginBottom:4 }}>• {w.english} <span style={{ color:C.accent }}>({w.count}回ミス)</span></div>
          ))}
        </div>
      )}
      ...
      {goals.filter(g=>!g.completed).map(g=>{
        ...
        return (
          <div key={g.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:14, marginBottom:10 }}>
            ...
            <div style={{ background:C.border, borderRadius:99, height:8, marginBottom:8, overflow:"hidden" }}>
              <div style={{ width:pct + "%", height:"100%", background:pct>=100?C.success:C.primary, borderRadius:99, transition:"width 0.4s" }} />
            </div>
      ...
      {goals.filter(g=>g.completed).map(g=>(
        <div key={g.id} style={{ background:C.successLight,border:`1px solid ${C.successMid}`,borderRadius:10,padding:12,marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <div><div style={{ fontSize:13,fontWeight:600,color:C.success }}>🏆 {g.title}</div><div style={{ fontSize:11,color:"#86efac" }}>{g.target} {g.unit}</div></div>
      ...
      {showAdd && (
        <Modal onClose={()=>setShowAdd(false)}>
          ...
          <ModalButtons
            ...
            onOk={()=>{ if(!newG.title.trim())return; setGoals(p=>[{...newG,id:uid(),current:0,completed:false,createdDate:today()},...p]); setNewG({title:"",target:30,unit:"表現",deadline:""}); setShowAdd(false); }}
            okLabel="追加する"
          />
        </Modal>
      )}
  ...
  if (mode === "result" && feedback) return (
    <div style={{ overflowY:"auto", padding:"14px 16px" }}>
      ...
      {feedback.strengths?.length > 0 && (
        <Section title="✨ 良かった点" color={C.success}>
          {feedback.strengths.map((s,i) => <div key={i} style={{ fontSize:12, color:"#166534", marginBottom:4 }}>• {s}</div>)}
        </Section>
      )}
      {feedback.improvements?.length > 0 && (
        <Section title="💡 より良い表現" color={C.warn}>
          {feedback.improvements.map((item,i) => (
            <div key={i} style={{ marginBottom:10, paddingBottom:10, borderBottom:i<feedback.improvements.length-1?`1px solid ${C.surface}`:"none" }}>
              <div style={{ fontSize:12, color:C.danger }}>❌ {item.original}</div>
              <div style={{ fontSize:12, color:C.success }}>✅ {item.better}</div>
              <div style={{ fontSize:11, color:C.muted, marginTop:3 }}>💡 {item.explanation}</div>
            </div>
          ))}
        </Section>
      )}
      {feedback.newPhrases?.length > 0 && (
        <Section title={`📚 使えるフレーズ (${feedback.newPhrases.length}件)`} color={C.primary}>
          {feedback.newPhrases.map((p,i) => (
            <div key={i} style={{ marginBottom:6, fontSize:12 }}>
              <span style={{ fontWeight:700 }}>{p.english}</span>
              <span style={{ color:C.muted }}> — {p.japanese}</span>
              {p.context && <div style={{ fontSize:11, color:C.subtle }}>{p.context}</div>}
            </div>
          ))}
        </Section>
      )}
      <button onClick={()=>{ setMode("list"); setSelected(null); setMessages([]); setFeedback(null); }} style={{ width:"100%", padding:14, borderRadius:12, border:"none", background:C.purple, color:"#fff", fontSize:15, fontWeight:700, cursor:"pointer", marginTop:8 }}>
        シナリオ一覧に戻る
      </button>
    </div>
  );
  ...
  if (mode === "play" && selected) return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      <div style={{ padding:"12px 16px", background:C.card, borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", gap:10 }}>
        <button onClick={()=>setMode("list")} style={{ background:"none", border:"none", cursor:"pointer", fontSize:20, color:C.muted }}>←</button>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:13, fontWeight:700, color:C.slate }}>{selected.title}</div>
          <div style={{ fontSize:10, color:C.subtle }}>{selected.category}</div>
        </div>
        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
          <button onClick={()=>setFeedbackMode(m=>m==="normal"?"practice":"normal")} style={{
            ...
            background: feedbackMode==="practice" ? C.purple : C.surface,
            color: feedbackMode==="practice" ? "#fff" : C.muted,
          }}>
          ...
          <button onClick={endAndGetFeedback} disabled={loading || messages.length < 2} style={{
            ...
            background: messages.length >= 2 ? C.success : C.border,
            color: messages.length >= 2 ? "#fff" : C.subtle,
          }}>終了</button>
        </div>
      </div>
      <div style={{ flex:1, overflowY:"auto", padding:"12px 16px" }}>
          ...
          <div key={i} style={{ marginBottom:12, display:"flex", justifyContent: m.role==="user"?"flex-end":"flex-start" }}>
            ...
            <div style={{
              ...
              {m.text.split("【フィードバック】").map((part, pi) => (
                <span key={pi} style={{ color: pi>0 ? C.purple : "inherit", fontSize: pi>0 ? 11 : 13, display: pi>0 ? "block" : "inline", marginTop: pi>0 ? 6 : 0, fontStyle: pi>0 ? "italic" : "normal" }}>
                  {pi>0 ? "💡 " : ""}{part}
                </span>
              ))}
            </div>
        ...
        {loading && (
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
            <div style={{ width:28, height:28, borderRadius:99, background:C.purple, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14 }}>🎭</div>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, borderBottomLeftRadius:4, padding:"10px 13px", fontSize:13, color:C.subtle }}>入力中…</div>
          </div>
        )}
        <div ref={messagesEndRef} />
      </div>
      <div style={{ padding:"6px 16px", background:C.surface, borderTop:`1px solid ${C.surface}` }}>
        <div style={{ fontSize:10, color:C.subtle }}>💡 ヒント: {selected.description}</div>
      </div>
          ...
          <div style={{ display:"flex", gap:8 }}>
            {[["normal","通常（終了後まとめて）"],["practice","練習（リアルタイム指摘）"]].map(([v,l])=>(
              <button key={v} onClick={()=>setFeedbackMode(v)} style={{
                flex:1, padding:"6px 0", borderRadius:8, border:`2px solid ${feedbackMode===v?C.purple:C.border}`,
                background: feedbackMode===v?C.purpleLight:C.card, cursor:"pointer", fontSize:10, fontWeight:feedbackMode===v?700:400,
                color: feedbackMode===v?C.purple:C.muted,
              }}>{l}</button>
            ))}
          </div>
        </div>
        <div style={{ display:"flex", gap:5, overflowX:"auto", paddingBottom:8 }}>
          {categories.map(c=>(
            <button key={c} onClick={()=>setFilterCat(c)} style={{
              padding:"3px 10px", borderRadius:99, border:"none", cursor:"pointer", whiteSpace:"nowrap",
              fontSize:11, fontWeight:600,
              background: filterCat===c ? C.purple : C.surface,
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"4px 16px 16px" }}>
        ...
        {filtered.map(s=>(
          <div key={s.id} onClick={()=>startScenario(s)} style={{
            background:C.card, border:`1px solid ${C.border}`, borderRadius:12, marginBottom:8, padding:"12px 14px", cursor:"pointer",
            ...
            <div style={{ flex:1 }}>
              <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:4 }}>
                ...
                <span style={{ fontSize:10, color:C.subtle }}>{s.category}</span>
              </div>
              <div style={{ fontSize:14, fontWeight:700, color:C.slate, marginBottom:2 }}>{s.title}</div>
              <div style={{ fontSize:11, color:C.muted }}>{s.description}</div>
            </div>
      ...
      {showAddScenario && (
        <Modal onClose={()=>setShowAddScenario(false)}>
          ...
          <Field label="難易度">
            <div style={{ display:"flex", gap:6 }}>
              {["初級","中級","上級"].map(d=>(
                <button key={d} onClick={()=>setNewScenario(s=>({...s,difficulty:d}))} style={{ flex:1, padding:"6px 0", borderRadius:8, border:"none", cursor:"pointer", fontSize:12, fontWeight:700, background:newScenario.difficulty===d?diffColor(d):C.surface, color:newScenario.difficulty===d?"#fff":diffColor(d) }}>{d}</button>
              ))}
            </div>
          </Field>

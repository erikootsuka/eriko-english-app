...
// ===================== VERSION =====================
const BUILD_VERSION = "2026-06-20-p8";
...
// 表現オブジェクトのusagesを正規化する。usages未設定（旧データ）の場合はデフォルトを補う。
...
function PhraseEditModal({ phrase, onSave, onClose, categories }) {
  ...
  function toggleUsage(id) {
    setEdited(p => {
      ...
      return { ...p, usages: has ? p.usages.filter(u => u !== id) : [...p.usages, id] };
    });
  }
      ...
      {showAdd && (
        <Modal onClose={() => setShowAdd(false)}>
          ...
          <Field label="用途（複数選択可）">
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {USAGE_OPTIONS.map(u => (
                <button key={u.id} type="button" onClick={() => setNewP(p => ({ ...p, usages: p.usages.includes(u.id) ? p.usages.filter(x => x !== u.id) : [...p.usages, u.id] }))} style={{
                  padding:"6px 12px", borderRadius:8, border:`1px solid ${newP.usages.includes(u.id) ? C.purple : C.border}`,
                  background: newP.usages.includes(u.id) ? C.purpleLight : C.surface,
                  color: newP.usages.includes(u.id) ? C.purple : C.muted,
                  fontSize:12, fontWeight: newP.usages.includes(u.id) ? 700 : 400, cursor:"pointer",
                }}>{u.label}</button>
              ))}
            </div>
          </Field>
  ...
  if (mode === "select") return (
    <div style={{ padding:"20px 16px" }}>
      ...
      <Field label="出題元">
        ...
        {source === "phrases" && (() => { const excluded = phrases.length - phrases.filter(hasJapanese).filter(p => hasUsage(p, "quiz")).length; return excluded > 0 ? (<div style={{ marginTop:8, padding:"8px 10px", background:C.warnLight, borderRadius:8, fontSize:11, color:C.warn }}>⚠️ 和訳なし・クイズ用途オフの表現 {excluded}件は出題対象から除外されます</div>) : null; })()}
    ...
    if (transSubMode === "select") return (
      <div style={{ padding:"16px", display:"flex", flexDirection:"column", height:"100%", overflowY:"auto" }}>
        ...
        <div style={{ background:C.card, borderRadius:12, padding:14, border:`1px solid ${C.border}` }}>
          ...
          <div style={{ fontSize:10, color:C.subtle, marginBottom:8 }}>用途に「✍️ 英作文・英訳」が設定された表現集の項目を原文として使えます（コピー不要・そのまま参照）</div>
          {writingPhraseSources.length === 0 ? (
            <div style={{ fontSize:12, color:C.subtle, textAlign:"center", padding:"12px 0" }}>用途に「✍️ 英作文・英訳」が設定された表現がまだありません。表現集の編集画面で用途を追加してください。</div>
  ...
  return (
    <div style={{ padding:"20px 16px" }}>
      ...
      <div style={{ background:C.card, borderRadius:12, padding:14, border:`1px solid ${C.border}`, marginBottom:14 }}>
        ...
        {missingCount > 0 && (<div style={{ fontSize:10, color:C.warn, marginTop:4 }}>⚠️ 和訳なし・練習用途オフの表現 {missingCount}件は対象外です</div>)}
      </div>

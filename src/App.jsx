...
Return ONLY one word: 初級, 中級, or 上級`;
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <ModalButtons
        onCancel={onClose}
        onOk={() => { onSave(edited); onClose(); }}
        okLabel="保存する"
        disabled={!edited.english.trim()}
      />
    </Modal>
  );
}
...
function PhrasesTab({ phrases, setPhrases }) {
  ...
  async function handleMultiFileSelect(e) {
    ...
    const readers = files.map(file => new Promise(resolve => {
      if (file.name.toLowerCase().endsWith(".docx")) {
        ...
        r.onload = async ev => {
          ...
          } catch { resolve({ name: file.name, text: "" }); }
        };
    ...
    Promise.all(readers).then(results => {
      ...
      setImportText(results.map(f => f.text).join("\n\n"));
    });
  }
...
Return ONLY valid JSON array with no other text: [{"english":"...","level":"初級"|"中級"|"上級"}]`;
        ...
        if (needsTranslation.length > 0) {
          ...
          const transSys = `You are a Japanese translator specializing in pharmaceutical and business English.
For each English phrase, provide a natural Japanese translation and brief context in Japanese.
Return ONLY valid JSON array: [{"english":"...","japanese":"...","context":"..."}]`;
          try {
        ...
        raw.forEach(p => {
          ...
          allPhrases.push({
            ...
            japanese: p.japanese || foundTrans?.japanese || "",
            context: p.context || foundTrans?.context || "",
            source: src.name,
          });
        });
      } catch {
  ...
  async function addPhraseWithLevel() {
    ...
    let japanese = newP.japanese;
    let context = newP.context;
    try {
      const sys = `You are an English level assessor and translator for a Japanese pharmaceutical professional.
Respond ONLY with valid JSON: {"level":"初級"|"中級"|"上級","japanese":"日本語訳","context":"使う場面(15文字以内)"}
Level criteria:
- 初級: Short, common words, junior high English, simple greetings/thanks
- 中級: Multiple clauses, business vocabulary, conditional forms
- 上級: Long/complex sentences, pharma/regulatory/legal terms, complex grammar`;
      const resp = await callClaude(sys, newP.english);
      const jsonMatch = resp.match(/\{[\s\S]*?\}/);
      if (jsonMatch) {
        const parsed = JSON.parse(jsonMatch[0]);
        if (LEVELS.includes(parsed.level)) level = parsed.level;
        if (!japanese && parsed.japanese) japanese = parsed.japanese;
        if (!context && parsed.context) context = parsed.context;
      }
    } catch {}
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      <div style={{ padding:"14px 16px 0" }}>
        ...
        <div style={{ display:"flex", gap:5, overflowX:"auto", paddingBottom:8, marginTop:4 }}>
          {PHRASE_CATS.map(c => (
            <button key={c} onClick={() => setCat(c)} style={{ padding:"3px 10px", borderRadius:99, border:"none", cursor:"pointer", whiteSpace:"nowrap", fontSize:11, fontWeight:600, background:cat===c?C.primary:C.surface, color:cat===c?"#fff":C.muted }}>{c}</button>
          ))}
        </div>
      </div>
            ...
            {expanded === p.id && (
              <div style={{ padding:"0 13px 11px", borderTop:`1px solid ${C.surface}` }}>
                ...
                <div style={{ display:"flex", gap:8, marginTop:8, alignItems:"center" }}>
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); setEditingPhrase(p); }}
                    style={{ background:C.primaryLight, border:`1px solid ${C.primaryMid}`, color:C.primary, borderRadius:6, padding:"2px 10px", fontSize:10, cursor:"pointer", fontWeight:600 }}
                  >編集</button>
                  <button
                    onClick={e => { e.stopPropagation(); setPhrases(prev => prev.filter(x => x.id !== p.id)); }}
                    style={{ background:"none", border:`1px solid ${C.dangerMid}`, color:C.danger, borderRadius:6, padding:"2px 8px", fontSize:10, cursor:"pointer" }}
                  >削除</button>
                </div>
              </div>
            )}
      ...
      {editingPhrase && (
        <PhraseEditModal
          ...
          onSave={updated => setPhrases(prev => prev.map(p => p.id === updated.id ? updated : p))}
      ...
      {showAdd && (
        <Modal onClose={() => setShowAdd(false)}>
          ...
          {[
            { k:"english", l:"英語 *", ph:"English phrase" },
            { k:"japanese", l:"日本語訳（空欄でAIが生成）", ph:"日本語訳" },
            { k:"context", l:"使う場面（空欄でAIが生成）", ph:"例: メールの締め" },
          ].map(f => (
            ...
            <>
              <label style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 14px", border:`2px dashed ${C.primaryMid}`, borderRadius:12, cursor:"pointer", background:C.primaryLight, marginBottom:10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:11, color:C.muted }}>.docx / .txt / .pdf に対応</div>
                </div>
              ...
              {importFiles.length > 0 && (
                <div style={{ background:C.successLight, border:`1px solid ${C.successMid}`, borderRadius:10, padding:10, marginBottom:10 }}>
                  ...
                  {importFiles.map((f, i) => <div key={i} style={{ fontSize:11, color:C.mid, marginBottom:2 }}>📄 {f.name}</div>)}
                </div>
              )}
              <div style={{ fontSize:11, color:C.subtle, textAlign:"center", margin:"6px 0" }}>または</div>
              <textarea
                value={importFiles.length > 0 ? "（ファイル選択済み）" : importText}
...
function VocabTab({ vocab, setVocab }) {
  ...
  async function addVocab() {
    ...
    let meaning = newV.meaning;
    let example = newV.example;
    let partOfSpeech = newV.partOfSpeech;
    try {
      const sys = `You are an English dictionary for a Japanese pharmaceutical professional.
Given a word or idiom, provide level, meaning, part of speech, and example sentence.
Return ONLY valid JSON: {"level":"初級"|"中級"|"上級","meaning":"日本語の意味","partOfSpeech":"名詞"|"動詞"|"形容詞"|"副詞"|"イディオム"|"フレーズ","example":"Example sentence using the word."}`;
      const resp = await callClaude(sys, newV.word);
      const jsonMatch = resp.match(/\{[\s\S]*?\}/);
      if (jsonMatch) {
        const parsed = JSON.parse(jsonMatch[0]);
        if (LEVELS.includes(parsed.level)) level = parsed.level;
        if (!meaning && parsed.meaning) meaning = parsed.meaning;
        if (!example && parsed.example) example = parsed.example;
        if (parsed.partOfSpeech) partOfSpeech = parsed.partOfSpeech;
      }
    } catch {}
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      <div style={{ padding:"14px 16px 0" }}>
        ...
        <div style={{ display:"flex", gap:5, overflowX:"auto", paddingBottom:8, marginTop:4 }}>
          {VOCAB_CATS.map(c => (<button key={c} onClick={() => setCat(c)} style={{ padding:"3px 10px", borderRadius:99, border:"none", cursor:"pointer", whiteSpace:"nowrap", fontSize:11, fontWeight:600, background:cat===c?C.accent:C.surface, color:cat===c?"#fff":C.muted }}>{c}</button>))}
        </div>
      </div>
          ...
          {filtered.length === 0 ? <div style={{ color:C.subtle, marginTop:40 }}>語彙がありません</div> : (
            <>
              ...
              <div onClick={() => setFlipped(v => !v)} style={{ width:"100%", minHeight:200, background:flipped?C.slate:C.card, border:`2px solid ${C.border}`, borderRadius:20, padding:24, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", cursor:"pointer", transition:"all 0.3s", textAlign:"center" }}>
                {!flipped ? (
                  <><div style={{ fontSize:11, color:C.subtle, marginBottom:8 }}>この単語の意味は？</div><div style={{ fontSize:24, fontWeight:800, color:C.slate }}>{card.word}</div><div style={{ fontSize:11, color:C.subtle, marginTop:8 }}>{card.partOfSpeech}</div><div style={{ fontSize:11, color:C.subtle, marginTop:16 }}>タップして答えを見る</div></>
                ) : (
                  <><div style={{ fontSize:18, fontWeight:800, color:"#fff", marginBottom:8 }}>{card.meaning}</div>{card.example && <div style={{ fontSize:12, color:C.subtle, lineHeight:1.5, fontStyle:"italic" }}>{card.example}</div>}</>
                )}
              </div>
      ...
      {showAdd && (
        <Modal onClose={() => setShowAdd(false)}>
          ...
          {[
            { k:"word", l:"英語 *", ph:"word / idiom" },
            { k:"meaning", l:"日本語の意味（空欄でAIが生成）", ph:"意味" },
            { k:"example", l:"例文（空欄でAIが生成）", ph:"Example sentence" },
          ].map(f => (<Field key={f.k} label={f.l}><input value={newV[f.k]} onChange={e => setNewV(v => ({ ...v, [f.k]: e.target.value }))} placeholder={f.ph} style={inp} /></Field>))}
...
function QuizTab({ phrases, vocab, setProgress, weaknesses, setWeaknesses }) {
  ...
  async function checkTyping() {
    ...
    setChecking(true);
    try {
      const sys = `You are an English teacher. Compare the student's answer to the correct answer and respond ONLY in JSON:
{"correct": true|false, "feedback": "brief feedback in Japanese", "correctAnswer": "the correct answer"}`;
  ...
  function answer(correct) {
    ...
    if (!correct) {
      setWeaknesses(prev => { const existing = prev.find(w => w.id === p.id); if (existing) return prev.map(w => w.id === p.id ? { ...w, count:w.count+1 } : w); return [...prev, { id:p.id, english:p.english, count:1 }]; });
    } else {
  ...
  return (
    <div style={{ padding:"14px 16px" }}>
      ...
      {quizType === "fillblank" && !revealed && (
        <div style={{ background:C.warnLight, border:`1px solid ${C.warnMid}`, borderRadius:10, padding:"8px 12px", marginBottom:10 }}>
          ...
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            ...
            {p.japanese && <span style={{ fontSize:11, color:C.slate }}>意味: <strong>{p.japanese}</strong></span>}
          </div>
        </div>
      )}
      ...
      {!revealed ? (
        inputMode === "typing" ? (
          <button onClick={checkTyping} disabled={checking || !userInput.trim()} style={{ width:"100%", padding:14, borderRadius:12, border:"none", background:checking?C.subtle:C.purple, color:"#fff", fontSize:15, fontWeight:700, cursor:"pointer" }}>{checking ? "チェック中…" : "チェックする"}</button>
        ) : (
...
function DiaryTab({ setPhrases, weaknesses }) {
  ...
  async function suggestTopic() {
    ...
    try {
      const sys = `You are an English writing coach for Eriko, a Japanese pharmaceutical regulatory affairs professional who also loves scuba diving.
Suggest ONE interesting diary topic for today in Japanese. Make it specific and personal, not generic.
Consider topics like: work situations, regulatory meetings, exhibitions, diving experiences, daily life observations, seasonal topics.
Return ONLY the topic suggestion in Japanese (1-2 sentences), no extra text.`;
      const resp = await callClaude(sys, `Today is ${today()}. Suggest a diary topic.`);
      setTopicSuggestion(resp.trim());
    } catch {
      setTopicSuggestion("今日の仕事で印象に残ったことや、最近考えていることを英語で書いてみましょう。");
    }
...
Analyze her diary entry and respond with ONLY a valid JSON object (no markdown, no explanation):
{
  "corrected": "full corrected diary text here",
  "corrections": [{"original": "wrong phrase", "corrected": "correct phrase", "explanation": "Japanese explanation"}],
  "patterns": [{"pattern": "grammar pattern name", "explanation": "Japanese explanation", "example": "example sentence"}],
  "newPhrases": [{"english": "useful phrase", "japanese": "Japanese meaning", "context": "when to use", "level": "初級"}],
...
- newPhrases: 3-5 useful phrases, level must be one of: 初級, 中級, 上級
    ...
    <div style={{ flex:1, overflowY:"auto", padding:"14px 16px" }}>
      ...
      {current.patterns?.length > 0 && (
        <Section title="📌 覚えてほしい文法パターン" color={C.primary}>
          {current.patterns.map((p, i) => (<div key={i} style={{ marginBottom:10 }}><div style={{ fontSize:13, fontWeight:700, color:C.primary }}>{p.pattern}</div><div style={{ fontSize:12, color:C.mid }}>{p.explanation}</div>{p.example && <div style={{ fontSize:12, color:C.muted, fontStyle:"italic", marginTop:2 }}>例: {p.example}</div>}</div>))}
        </Section>
      )}
      {current.newPhrases?.length > 0 && (
        <Section title={`📚 表現集に追加しました (${current.newPhrases.length}件)`} color={C.purple}>
          {current.newPhrases.map((p, i) => (<div key={i} style={{ marginBottom:6, fontSize:12 }}><span style={{ fontWeight:700 }}>{p.english}</span>{p.japanese && <span style={{ color:C.muted }}> — {p.japanese}</span>}</div>))}
        </Section>
      )}
    ...
    <div style={{ overflowY:"auto", padding:"16px" }}>
      ...
      {showDownload && (
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:14, marginBottom:14 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:10 }}>
            ...
            ].map(b => (<button key={b.label} onClick={b.onClick} style={{ background:C.card, border:`1px solid ${b.color}33`, borderRadius:10, padding:"10px 8px", cursor:"pointer", textAlign:"left" }}><div style={{ fontSize:12, fontWeight:700, color:b.color }}>{b.label}</div><div style={{ fontSize:10, color:C.subtle, marginTop:2 }}>{b.sub}</div></button>))}
          </div>
      ...
      <div style={{ background:`linear-gradient(135deg,${C.slate},#334155)`, borderRadius:14, padding:16, marginBottom:14, color:"#fff" }}>
        ...
        <div style={{ display:"flex", gap:16 }}>
          {[{l:"今日の演習",v:todayStudied},{l:"連続日数",v:`${streak}日`},{l:"苦手表現",v:`${weaknesses.length}件`}].map(s => (<div key={s.l}><div style={{ fontSize:20, fontWeight:800 }}>{s.v}</div><div style={{ fontSize:10, color:C.subtle }}>{s.l}</div></div>))}
        </div>
      </div>
      ...
      {weaknesses.length > 0 && (
        <div style={{ background:C.accentLight, border:`1px solid ${C.accentMid}`, borderRadius:12, padding:14, marginBottom:14 }}>
          ...
          {weaknesses.slice(0,5).map(w => (<div key={w.id} style={{ fontSize:12, color:"#92400e", marginBottom:4 }}>• {w.english} <span style={{ color:C.accent }}>({w.count}回ミス)</span></div>))}
        </div>
      )}
...
function RoleplayTab() {
  ...
  async function startScenario(scenario) {
    ...
    try {
      const resp = await callClaude(scenario.systemPrompt, "Start the roleplay now. Begin with your opening line.");
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const sys = feedbackMode === "practice"
  ...
  async function endAndGetFeedback() {
    ...
    try {
      ...
      const sys = `You are an English teacher reviewing a roleplay conversation. Analyze Eriko's English and provide feedback in Japanese. Return ONLY valid JSON with no extra text:
{"overall":"総合評価コメント","score":1-10,"strengths":["良かった点"],"improvements":[{"original":"Erikoの表現","better":"より良い表現","explanation":"説明"}],"newPhrases":[{"english":"フレーズ","japanese":"意味","context":"使う場面"}]}`;
    ...
    <div style={{ overflowY:"auto", padding:"14px 16px" }}>
      ...
      {feedback.improvements?.length > 0 && (
        <Section title="💡 より良い表現" color={C.warn}>
          {feedback.improvements.map((item,i) => (<div key={i} style={{ marginBottom:10, paddingBottom:10, borderBottom:i<feedback.improvements.length-1?`1px solid ${C.surface}`:"none" }}><div style={{ fontSize:12, color:C.danger }}>❌ {item.original}</div><div style={{ fontSize:12, color:C.success }}>✅ {item.better}</div><div style={{ fontSize:11, color:C.muted, marginTop:3 }}>💡 {item.explanation}</div></div>))}
        </Section>
      )}
      {feedback.newPhrases?.length > 0 && (
        <Section title={`📚 使えるフレーズ (${feedback.newPhrases.length}件)`} color={C.primary}>
          {feedback.newPhrases.map((p,i) => (<div key={i} style={{ marginBottom:6, fontSize:12 }}><span style={{ fontWeight:700 }}>{p.english}</span><span style={{ color:C.muted }}> — {p.japanese}</span>{p.context && <div style={{ fontSize:11, color:C.subtle }}>{p.context}</div>}</div>))}
        </Section>
      )}
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      <div style={{ padding:"14px 16px 0" }}>
        ...
        <div style={{ background:C.surface, borderRadius:10, padding:10, marginBottom:10, border:`1px solid ${C.border}` }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            {[["normal","通常（終了後まとめて）"],["practice","練習（リアルタイム指摘）"]].map(([v,l]) => (<button key={v} onClick={() => setFeedbackMode(v)} style={{ flex:1, padding:"6px 0", borderRadius:8, border:`2px solid ${feedbackMode===v?C.purple:C.border}`, background:feedbackMode===v?C.purpleLight:C.card, cursor:"pointer", fontSize:10, fontWeight:feedbackMode===v?700:400, color:feedbackMode===v?C.purple:C.muted }}>{l}</button>))}
          </div>
        </div>
        <div style={{ display:"flex", gap:5, overflowX:"auto", paddingBottom:8 }}>
          {categories.map(c => (<button key={c} onClick={() => setFilterCat(c)} style={{ padding:"3px 10px", borderRadius:99, border:"none", cursor:"pointer", whiteSpace:"nowrap", fontSize:11, fontWeight:600, background:filterCat===c?C.purple:C.surface, color:filterCat===c?"#fff":C.muted }}>{c}</button>))}
        </div>
      </div>
      <div style={{ flex:1, overflowY:"auto", padding:"4px 16px 16px" }}>
      ...
      {showAddScenario && (
        <Modal onClose={() => setShowAddScenario(false)}>
          ...
          <Field label="難易度">
            <div style={{ display:"flex", gap:6 }}>
              {["初級","中級","上級"].map(d => (<button key={d} onClick={() => setNewScenario(s => ({ ...s, difficulty:d }))} style={{ flex:1, padding:"6px 0", borderRadius:8, border:"none", cursor:"pointer", fontSize:12, fontWeight:700, background:newScenario.difficulty===d?diffColor(d):C.surface, color:newScenario.difficulty===d?"#fff":diffColor(d) }}>{d}</button>))}
            </div>
          </Field>

...
const BACKUP_META_KEY = "eriko_backup_meta";
...
// Fisher-Yatesアルゴリズムで配列をシャッフルする（フラッシュカードの出題順ランダム化用）
...
const USAGE_OPTIONS = [
  ...
  { id: "writing", label: "✍️ 英作文・英訳" },
];
const DEFAULT_USAGES = ["quiz", "practice"];

const WRITING_LENGTH_THRESHOLD = { english: 50, japanese: 25 };

function isLongEnoughForWriting(phrase) {
  const enLen = (phrase.english || "").trim().length;
  const jaLen = (phrase.japanese || "").trim().length;
  return enLen >= WRITING_LENGTH_THRESHOLD.english || jaLen >= WRITING_LENGTH_THRESHOLD.japanese;
}

function computeAutoUsages(phrase) {
  const longEnough = isLongEnoughForWriting(phrase);
  return longEnough ? ["practice", "writing"] : ["practice", "quiz"];
}

function normalizeUsages(phrase) {
  if (Array.isArray(phrase.usages) && phrase.usages.length > 0) return phrase.usages;
  return computeAutoUsages(phrase);
}
function hasUsage(phrase, usageId) {
  return normalizeUsages(phrase).includes(usageId);
}
// ===================== TTS =====================
...
function speakWithWordHighlight(text, { rate = 0.9, onHighlight, onEnd } = {}) {
  ...
  function startFallbackHighlight() {
    ...
    const totalDurationMs = Math.max(600, text.length * (110 / rate));
    let elapsed = 0;
    words.forEach((_, i) => {
...
function PhraseEditModal({ phrase, onSave, onClose, categories }) {
  const [edited, setEdited] = useState({ ...phrase, usages: normalizeUsages(phrase) });
  ...
  function toggleUsage(id) {
    setEdited(p => {
      const has = p.usages.includes(id);
      let next = has ? p.usages.filter(u => u !== id) : [...p.usages, id];
      if (!has && id === "writing") next = next.filter(u => u !== "quiz");
      if (!has && id === "quiz") next = next.filter(u => u !== "writing");
      return { ...p, usages: next };
    });
  }
...
Return ONLY one word: 初級, 中級, or 上級`;
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <Field label="用途（複数選択可・クイズと英作文は併用不可）">
        <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
          {USAGE_OPTIONS.map(u => (
            <button key={u.id} onClick={() => toggleUsage(u.id)} type="button" style={{
              padding:"6px 12px", borderRadius:8, border:`1px solid ${edited.usages.includes(u.id) ? C.purple : C.border}`,
              background: edited.usages.includes(u.id) ? C.purpleLight : C.surface,
              color: edited.usages.includes(u.id) ? C.purple : C.muted,
              fontSize:12, fontWeight: edited.usages.includes(u.id) ? 700 : 400, cursor:"pointer",
            }}>{u.label}</button>
          ))}
        </div>
        {edited.usages.length === 0 && (<div style={{ fontSize:10, color:C.warn, marginTop:4 }}>⚠️ 用途を1つ以上選んでください（どの機能にも出題されなくなります）</div>)}
      </Field>
      <Field label="難易度">
...
function PhrasesTab({ phrases, setPhrases }) {
  const [viewMode, setViewMode] = useState("list"); // list | flash
  ...
  const [flashOrder, setFlashOrder] = useState([]); // ランダム化されたカード出題順（idの配列）
  const [flipped, setFlipped] = useState(false);
  const [direction, setDirection] = useState("ja2en"); // ja2en | en2ja | random
  const [cardDirection, setCardDirection] = useState("ja2en");
  ...
  const [newP, setNewP] = useState({ japanese:"", english:"", context:"", category: phraseCats[0] || "ビジネス挨拶", level:"初級", usages: [...DEFAULT_USAGES] });
  ...
  // カードが切り替わるたびに、ランダムモードならその回の表示方向を抽選し直す
  useEffect(() => {
    if (direction === "random") {
      setCardDirection(Math.random() < 0.5 ? "ja2en" : "en2ja");
    } else {
      setCardDirection(direction);
    }
  }, [flashIdx, direction]);

  // フラッシュカードの出題順をランダム化する。
  // フィルター後のカード集合（filtered）が変わったとき、またはカードモードに
  // 切り替えたタイミングでシャッフルし直す（リスト表示中は再計算しない）。
  const filteredIdsKey = filtered.map(p => p.id).join(",");
  useEffect(() => {
    if (viewMode !== "flash") return;
    setFlashOrder(shuffleArray(filtered.map(p => p.id)));
    setFlashIdx(0);
  }, [viewMode, filteredIdsKey]);

  const flashCards = (flashOrder.length === filtered.length)
    ? flashOrder.map(id => filtered.find(p => p.id === id)).filter(Boolean)
    : filtered;
...
Return ONLY valid JSON array with no other text: [{"english":"...","level":"初級"|"中級"|"上級"}]`;
        ...
        raw.forEach(p => {
          ...
          const japanese = p.japanese && p.japanese.trim() ? p.japanese : (foundTrans?.japanese || "");
          const context = p.context && p.context.trim() ? p.context : (foundTrans?.context || "");
          allPhrases.push({
            ...p,
            ...
            context,
            source: src.name,
            usages: computeAutoUsages({ english: p.english, japanese }),
          });
        });
      } catch {
        raw.forEach(p => allPhrases.push({ ...p, source: src.name, usages: computeAutoUsages(p) }));
      }
    }
    const seen = new Set();
  ...
  async function addPhraseWithLevel() {
    ...
    const autoUsages = computeAutoUsages({ english: newP.english, japanese });
    setPhrases(prev => [{ ...newP, level, japanese, context, usages: autoUsages, id:uid(), source:"手動追加", addedDate:today() }, ...prev]);
    setNewP({ japanese:"", english:"", context:"", category:"ビジネス挨拶", level:"初級", usages: [...DEFAULT_USAGES] });
    setShowAdd(false);
  }
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      <div style={{ padding:"14px 16px 0" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
          ...
          <div style={{ display:"flex", gap:6 }}>
            <button onClick={() => setViewMode(viewMode === "list" ? "flash" : "list")} style={{ background:C.surface, border:"none", borderRadius:8, padding:"5px 8px", fontSize:11, cursor:"pointer", color:C.mid, fontWeight:600 }}>{viewMode === "list" ? "🃏 カード" : "📋 リスト"}</button>
      ...
      {viewMode === "flash" ? (
        <div style={{ flex:1, padding:"16px", display:"flex", flexDirection:"column", alignItems:"center", overflowY:"auto" }}>
          <div style={{ display:"flex", gap:6, marginBottom:12, width:"100%" }}>
            {[
              ...
              <button key={opt.id} onClick={() => { setDirection(opt.id); setFlipped(false); }} style={{
                flex:1, padding:"7px 0", borderRadius:8, border:"none", cursor:"pointer",
                fontSize:11, fontWeight:700,
                background: direction === opt.id ? C.primary : C.surface,
                color: direction === opt.id ? "#fff" : C.muted,
              }}>{opt.label}</button>
            ))}
          </div>
          {flashCards.length === 0 ? <div style={{ color:C.subtle, marginTop:40 }}>表現がありません</div> : (() => {
            const card = flashCards[flashIdx % flashCards.length];
            const isEn2ja = cardDirection === "en2ja";
            return (
              <>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                  <div style={{ fontSize:11, color:C.subtle }}>{(flashIdx % flashCards.length) + 1} / {flashCards.length}</div>
                  {direction === "random" && (
                    <span style={{ fontSize:9, padding:"2px 8px", borderRadius:99, background:C.purpleLight, color:C.purple, fontWeight:700 }}>{isEn2ja ? "🇬🇧→🇯🇵" : "🇯🇵→🇬🇧"}</span>
                  )}
                  <span style={{ fontSize:9, padding:"2px 8px", borderRadius:99, background:levelBg(card.level), color:levelColor(card.level), fontWeight:700 }}>{card.level}</span>
                </div>
                <div onClick={() => setFlipped(v => !v)} style={{ width:"100%", minHeight:200, background:flipped?C.slate:C.card, border:`2px solid ${C.border}`, borderRadius:20, padding:24, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", cursor:"pointer", transition:"all 0.3s", textAlign:"center" }}>
                  {!flipped ? (
                    isEn2ja ? (
                      <>
                        <div style={{ fontSize:11, color:C.subtle, marginBottom:8 }}>この英語の意味は？</div>
                        <div style={{ fontSize:18, fontWeight:800, color:C.slate, lineHeight:1.5 }}>{card.english}</div>
                        <div style={{ fontSize:11, color:C.subtle, marginTop:16 }}>タップして答えを見る</div>
                      </>
                    ) : (
                      <>
                        <div style={{ fontSize:11, color:C.subtle, marginBottom:8 }}>次の日本語を英語で？</div>
                        <div style={{ fontSize:18, fontWeight:800, color:C.slate, lineHeight:1.5 }}>{card.japanese || "（和訳なし）"}</div>
                        <div style={{ fontSize:11, color:C.subtle, marginTop:16 }}>タップして答えを見る</div>
                      </>
                    )
                  ) : (
                    isEn2ja ? (
                      <>
                        <div style={{ fontSize:18, fontWeight:800, color:"#fff", marginBottom:8 }}>{card.japanese || "（和訳なし）"}</div>
                        {card.context && <div style={{ fontSize:12, color:C.subtle, lineHeight:1.5, fontStyle:"italic" }}>💬 {card.context}</div>}
                      </>
                    ) : (
                      <>
                        <div style={{ fontSize:16, fontWeight:800, color:"#fff", marginBottom:8, lineHeight:1.5 }}>{card.english}</div>
                        {card.context && <div style={{ fontSize:12, color:C.subtle, lineHeight:1.5, fontStyle:"italic" }}>💬 {card.context}</div>}
                      </>
                    )
                  )}
                </div>
                <div style={{ fontSize:10, color:C.subtle, marginTop:10 }}>{card.category}</div>
                <div style={{ display:"flex", gap:10, marginTop:16, width:"100%" }}>
                  <button onClick={() => { setFlashIdx(i => (i - 1 + flashCards.length) % flashCards.length); setFlipped(false); }} style={{ flex:1, padding:12, borderRadius:10, border:`1px solid ${C.border}`, background:C.card, fontSize:18, cursor:"pointer" }}>◀</button>
                  <button onClick={() => { setFlashIdx(i => (i + 1) % flashCards.length); setFlipped(false); }} style={{ flex:1, padding:12, borderRadius:10, border:"none", background:C.primary, color:"#fff", fontSize:18, cursor:"pointer", fontWeight:700 }}>次へ ▶</button>
                </div>
              </>
            );
          })()}
          ...
          <div key={p.id} style={{ background:C.card, border:`1px solid ${isMissing ? C.warnMid : C.border}`, borderRadius:12, marginBottom:8, overflow:"hidden" }}>
            <div onClick={() => setExpanded(expanded === p.id ? null : p.id)} style={{ padding:"11px 13px", cursor:"pointer" }}>
              ...
              <div style={{ display:"flex", gap:5, marginTop:6, flexWrap:"wrap" }}>
                ...
                {USAGE_OPTIONS.filter(u => hasUsage(p, u.id)).map(u => (
                  <span key={u.id} style={{ fontSize:9, padding:"2px 8px", borderRadius:99, background:C.purpleLight, color:C.purple, fontWeight:600 }}>{u.label}</span>
                ))}
              </div>
            </div>
            {expanded === p.id && (
      ...
      {showAdd && (
        <Modal onClose={() => setShowAdd(false)}>
          ...
          <p style={{ fontSize:11, color:C.muted, margin:"0 0 12px" }}>英語を入力するとAIが日本語訳・難易度を自動生成します。用途（クイズ/英作文）は文字数に応じて自動設定されます。</p>
                    ...
                    <div key={p.id} style={{ padding:"8px 0", borderBottom:`1px solid ${C.surface}`, fontSize:12 }}>
                      ...
                      <input
                        ...
                        onChange={e => {
                          ...
                          setImportResult(prev => prev.map((x, i) => i === idx ? { ...x, japanese: val, usages: computeAutoUsages({ english: x.english, japanese: val }) } : x));
                        }}
                        placeholder="日本語訳を入力…"
                        style={{
...
// ===================== VOCAB EDIT MODAL =====================
function VocabEditModal({ vocabItem, onSave, onClose }) {
  const [edited, setEdited] = useState({ ...vocabItem });
  const [autoLevelLoading, setAutoLevelLoading] = useState(false);
  const inp = { width:"100%", padding:"8px 10px", borderRadius:8, border:`1px solid ${C.border}`, fontSize:13, boxSizing:"border-box", outline:"none" };
  ...
  async function autoLevel() {
    ...
    try {
      const sys = `You are an English level assessor for a Japanese pharmaceutical professional.
Classify the difficulty using STRICT criteria:
- 初級: Common everyday words, junior high school English level
- 中級: Business-specific vocabulary or moderately advanced everyday words
- 上級: Regulatory/pharmaceutical/legal/scientific terminology or advanced/rare vocabulary
Return ONLY one word: 初級, 中級, or 上級`;
      const resp = (await callClaude(sys, edited.word)).trim();
      if (LEVELS.includes(resp)) setEdited(e => ({ ...e, level: resp }));
    } catch {}
    setAutoLevelLoading(false);
  }
  ...
  return (
    <Modal onClose={onClose}>
      <h4 style={{ margin:"0 0 12px", fontSize:15 }}>語彙を編集</h4>
      <Field label="英語 *">
        <input value={edited.word} onChange={e => setEdited(p => ({ ...p, word: e.target.value }))} style={inp} />
      </Field>
      <Field label="日本語の意味">
        <input value={edited.meaning} onChange={e => setEdited(p => ({ ...p, meaning: e.target.value }))} placeholder="意味" style={inp} />
      </Field>
      <Field label="例文">
        <input value={edited.example} onChange={e => setEdited(p => ({ ...p, example: e.target.value }))} placeholder="Example sentence" style={inp} />
      </Field>
      <Field label="品詞">
        <select value={edited.partOfSpeech} onChange={e => setEdited(p => ({ ...p, partOfSpeech: e.target.value }))} style={inp}>
      ...
      <ModalButtons onCancel={onClose} onOk={() => { onSave(edited); onClose(); }} okLabel="保存する" disabled={!edited.word.trim()} />
    </Modal>
  );
}
...
function VocabTab({ vocab, setVocab, autoOpen }) {
  ...
  const [flashOrder, setFlashOrder] = useState([]); // ランダム化されたカード出題順（idの配列）
  const [flipped, setFlipped] = useState(false);
  const [direction, setDirection] = useState("en2ja"); // en2ja | ja2en | random
  const [cardDirection, setCardDirection] = useState("en2ja"); // ランダムモード時に各カードへ割り当てる実際の方向
  ...
  useEffect(() => {
    if (direction === "random") {
      setCardDirection(Math.random() < 0.5 ? "en2ja" : "ja2en");
    } else {
      setCardDirection(direction);
    }
  }, [flashIdx, direction]);
  ...
  // フィルター後のカード集合（filtered）が変わったとき、またはカードモードに
  // 切り替えたタイミングでシャッフルし直す（リスト表示中は再計算しない）。
  const filteredIdsKey = filtered.map(v => v.id).join(",");
  useEffect(() => {
    if (mode !== "flash") return;
    setFlashOrder(shuffleArray(filtered.map(v => v.id)));
    setFlashIdx(0);
  }, [mode, filteredIdsKey]);

  const flashCards = (flashOrder.length === filtered.length)
    ? flashOrder.map(id => filtered.find(v => v.id === id)).filter(Boolean)
    : filtered;
  ...
  const card = flashCards[flashIdx % Math.max(1, flashCards.length)];
  const isJa2en = cardDirection === "ja2en";
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      {mode === "flash" ? (
        <div style={{ flex:1, padding:"16px", display:"flex", flexDirection:"column", alignItems:"center" }}>
          ...
          {flashCards.length === 0 ? <div style={{ color:C.subtle, marginTop:40 }}>語彙がありません</div> : (
            <>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                <div style={{ fontSize:11, color:C.subtle }}>{(flashIdx % flashCards.length) + 1} / {flashCards.length}</div>
                {direction === "random" && (
              ...
              <div onClick={() => setFlipped(v => !v)} style={{ width:"100%", minHeight:200, background:flipped?C.slate:C.card, border:`2px solid ${C.border}`, borderRadius:20, padding:24, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", cursor:"pointer", transition:"all 0.3s", textAlign:"center" }}>
                {!flipped ? (
                  isJa2en ? (
                    <>
                      <div style={{ fontSize:11, color:C.subtle, marginBottom:8 }}>この意味の英語は？</div>
                      <div style={{ fontSize:22, fontWeight:800, color:C.slate }}>{card.meaning}</div>
                      <div style={{ fontSize:11, color:C.subtle, marginTop:8 }}>{card.partOfSpeech}</div>
                      <div style={{ fontSize:11, color:C.subtle, marginTop:16 }}>タップして答えを見る</div>
                    </>
                  ) : (
                    <>
                      <div style={{ fontSize:11, color:C.subtle, marginBottom:8 }}>この単語の意味は？</div>
                      <div style={{ fontSize:24, fontWeight:800, color:C.slate }}>{card.word}</div>
                      <div style={{ fontSize:11, color:C.subtle, marginTop:8 }}>{card.partOfSpeech}</div>
                      <div style={{ fontSize:11, color:C.subtle, marginTop:16 }}>タップして答えを見る</div>
                    </>
                  )
                  ...
                  isJa2en ? (
                    <>
                      <div style={{ fontSize:18, fontWeight:800, color:"#fff", marginBottom:8 }}>{card.word}</div>
                      {card.example && (
                        <div style={{ display:"flex", alignItems:"center", gap:6, justifyContent:"center", marginTop:2 }}>
                          <div style={{ fontSize:12, color:C.subtle, lineHeight:1.5, fontStyle:"italic" }}>{card.example}</div>
                          <button onClick={e => { e.stopPropagation(); speak(card.example); }} style={{ background:"rgba(255,255,255,0.15)", border:"none", borderRadius:99, width:22, height:22, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, cursor:"pointer", flexShrink:0 }} title="例文の発音を聞く">🔊</button>
                        </div>
                      )}
                    </>
                  ) : (
      ...
      {editingVocab && (
        <VocabEditModal
          ...
          onSave={updated => setVocab(prev => prev.map(v => v.id === updated.id ? updated : v))}
...
function QuizTab({ phrases, vocab, setProgress, weaknesses, setWeaknesses }) {
  ...
  const allItems = source === "phrases" ? phrases.filter(hasJapanese).filter(p => hasUsage(p, "quiz")) : source === "vocab" ? vocab.filter(v => v.meaning && v.meaning.trim()).map(v => ({ ...v, english:v.word, japanese:v.meaning, context:v.example })) : diaryPool;
  ...
  if (mode === "select") return (
    <div style={{ padding:"20px 16px" }}>
      ...
      <Field label="出題元">
        ...
        {source === "phrases" && (() => { const excluded = phrases.length - phrases.filter(hasJapanese).filter(p => hasUsage(p, "quiz")).length; return excluded > 0 ? (<div style={{ marginTop:8, padding:"8px 10px", background:C.warnLight, borderRadius:8, fontSize:11, color:C.warn }}>⚠️ 和訳なし・クイズ用途オフ（英作文用に分類された長文など）の表現 {excluded}件は出題対象から除外されます</div>) : null; })()}
...
function DiaryTab({ setPhrases, weaknesses }) {
  ...
  async function submitDiary() {
    ...
    try {
      ...
      const sys = `You are Eriko's English writing companion. Eriko is a Japanese pharmaceutical regulatory affairs professional who writes an English diary to improve her language skills. You are not a strict teacher grading her work — you are a supportive partner who walks alongside her learning journey. Analyze her diary entry and respond with ONLY a valid JSON object (no markdown, no explanation): { "comment": "a short, warm reaction to the CONTENT of her diary in Japanese (1-2 sentences) — react like a friend who actually read and enjoyed hearing about her day/topic, BEFORE getting into corrections", "corrected": "full corrected diary text here", "corrections": [{"original": "wrong phrase", "corrected": "correct phrase", "explanation": "Japanese explanation, phrased supportively like 'こう言うとより自然だよ' rather than declaring it simply wrong"}], "patterns": [{"pattern": "grammar pattern name", "explanation": "Japanese explanation", "example": "example sentence"}], "newPhrases": [{"english": "useful phrase", "japanese": "Japanese meaning", "context": "when to use", "level": "初級"}], "weakPoints": ["weakness description in Japanese, phrased as friendly encouragement rather than criticism"] } Rules: - ALL SIX top-level keys (comment, corrected, corrections, patterns, newPhrases, weakPoints) MUST always be present in your response, even if some of them are empty arrays [] - comment: ALWAYS include this, even for very short entries. React specifically to something she mentioned (an event, a feeling, a place, a person) — show you actually read it, don't just say "良い日記ですね". Keep it brief and genuine, not exaggerated. - corrections: list ALL actual errors found, thoroughly and precisely — do not hold back on identifying grammar issues. List every one (empty array [] only if there truly are none). The tone of each explanation should be that of an encouraging companion pointing out a better way to say something, never harsh or scolding, but the content and completeness of the grammar feedback itself must not be reduced. - patterns: even if the diary is short or has no errors, ALWAYS identify 1-3 useful grammar patterns or sentence structures she used well or could use, with a Japanese explanation and example - newPhrases: even if the diary is short, ALWAYS suggest 2-5 useful phrases related to the diary's topic or her work (pharmaceutical regulatory affairs) that she could use in similar future entries, level must be one of: 初級, 中級, 上級 - weakPoints: even if grammatically correct, ALWAYS give 1-3 constructive suggestions for improvement (e.g. add more detail, use more advanced vocabulary, vary sentence structure), framed gently as "次はこうしてみるともっと良くなるよ" rather than as a deficiency - Her known weak areas: ${weakList || "none yet"} - Overall tone: warm, encouraging, like a companion cheering her on — but this must NEVER reduce the thoroughness or accuracy of the grammar corrections themselves. The grammar feedback should be just as complete and rigorous as before; only the delivery and packaging changes. - CRITICAL JSON FORMATTING: if the diary text itself contains quoted words or nicknames (e.g. she wrote something like "Oyakata"), do NOT reproduce the quote marks inside your JSON string values. Instead, rewrite it without quote marks (e.g. the nickname Oyakata) or escape every double quote as \\" with a backslash. Never output a bare unescaped " character inside any JSON string value. Do not truncate or cut off your response; always output the complete JSON object with the closing brace.`;
      const result = await callClaudeJSON(sys, draft, { requiredKeys: ["comment", "corrected", "corrections", "patterns", "newPhrases", "weakPoints"] });
      result.corrections = result.corrections || []; result.patterns = result.patterns || []; result.newPhrases = result.newPhrases || []; result.weakPoints = result.weakPoints || []; result.corrected = result.corrected || draft; result.comment = result.comment || "";
      ...
      if (result.newPhrases?.length) { setPhrases(prev => [...result.newPhrases.map(p => ({ ...p, id:uid(), category:"日記表現", source:"日記添削", addedDate:today(), usages: computeAutoUsages(p) })), ...prev]); }
  ...
  if (mode === "view" && current) return (
    <div style={{ flex:1, overflowY:"auto", padding:"14px 16px", height:"100%" }}>
      ...
      {current.comment && (
        <div style={{ background:C.purpleLight, border:`1px solid #ddd6fe`, borderRadius:12, padding:14, marginBottom:12 }}>
          <div style={{ fontSize:12, color:C.purple, lineHeight:1.6 }}>💬 {current.comment}</div>
        </div>
      )}
...
function PracticeTab({ phrases }) {
  ...
  const [cat, setCat] = useState("すべて");
  const [shadowLevel, setShadowLevel] = useState("すべて"); // シャドーイング専用のレベル絞り込み（ディクテーションと同様に追加）
  ...
  const withJapanese = phrases.filter(hasJapanese).filter(p => hasUsage(p, "practice"));
  ...
  const shadowFiltered = shadowLevel === "すべて" ? filtered : filtered.filter(p => p.level === shadowLevel);
  ...
  function startShadowing() {
    const p = pickRandom(shadowFiltered);
    if (!p) return;
    setCurrentPhrase(p);
    setSubMode("listen");
    setHighlightIdx(-1);
  ...
  if (mode === "translation") {
    ...
    if (transSubMode === "select") return (
      <div style={{ padding:"16px", display:"flex", flexDirection:"column", height:"100%", overflowY:"auto" }}>
        ...
        <div style={{ background:C.card, borderRadius:12, padding:14, border:`1px solid ${C.border}` }}>
          <div style={{ fontSize:13, fontWeight:800, color:C.slate, marginBottom:4 }}>📚 表現集から使う</div>
          <div style={{ fontSize:10, color:C.subtle, marginBottom:8 }}>ある程度長さのある表現（用途に「✍️ 英作文・英訳」が設定された項目）を原文として使えます（コピー不要・そのまま参照）</div>
          {writingPhraseSources.length === 0 ? (
            <div style={{ fontSize:12, color:C.subtle, textAlign:"center", padding:"12px 0" }}>用途に「✍️ 英作文・英訳」が設定された表現がまだありません。ある程度長さのある表現を追加すると自動的にここに表示されます。</div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        <div style={{ background:C.card, border:`2px solid ${C.primary}`, borderRadius:16, padding:20 }}>
          ...
          <div style={{ display:"flex", gap:6, marginBottom:12 }}>
            {["すべて", ...LEVELS].map(l => (
              <button key={l} onClick={() => setShadowLevel(l)} style={{ flex:1, padding:"6px 0", borderRadius:8, border:"none", cursor:"pointer", fontSize:11, fontWeight:700, background: shadowLevel===l ? (l==="すべて"?C.slate:levelColor(l)) : C.surface, color: shadowLevel===l ? "#fff" : (l==="すべて"?C.muted:levelColor(l)) }}>{l}</button>
            ))}
          </div>
          <div style={{ fontSize:11, color:C.subtle, marginBottom:10 }}>{shadowFiltered.length}件の表現から出題</div>
          <button onClick={startShadowing} disabled={shadowFiltered.length === 0} style={{ width:"100%", padding:12, borderRadius:10, border:"none", background: shadowFiltered.length > 0 ? C.primary : C.subtle, color:"#fff", fontSize:14, fontWeight:700, cursor: shadowFiltered.length > 0 ? "pointer" : "not-allowed" }}>🎤 シャドーイングを始める</button>
        </div>
      ...
      const sys = `You are an English learning coach analyzing Eriko's progress. She is a Japanese pharmaceutical regulatory affairs professional learning English. Analyze her learning data and generate a comprehensive growth report in Japanese. Return ONLY valid JSON:
{
      ...
      try {
        ...
        const parsed = rows.filter(r => r.english && r.english.trim()).map(r => {
          ...
          return {
            ...
            usages: computeAutoUsages({ english: r.english || "", japanese }),
          };
        });
  ...
  function reapplyUsageRules() {
    const targetCount = phrases.length;
    if (targetCount === 0) { setRestoreMsg("⚠️ 表現集が空のため、適用対象がありません。"); return; }
    if (!window.confirm(`既存の表現集 ${targetCount}件すべてに、新しい用途タグの自動判定ルール（長さに応じてクイズ/英作文を自動振り分け）を再適用します。\n個別に手動調整していた用途設定も上書きされます。よろしいですか？`)) return;
    setPhrases(prev => prev.map(p => ({ ...p, usages: computeAutoUsages(p) })));
    setRestoreMsg(`✅ ${targetCount}件の用途タグを再判定しました`);
  }

  const [idbStatus, setIdbStatus] = useState(null);
      ...
      {showDownload && (
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:14, marginBottom:14 }}>
          ...
          <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:14, marginBottom:14 }}>
            ...
            <div style={{ fontSize:10, color:C.muted, marginBottom:10 }}>既存の表現集すべてに、長さに応じたクイズ/英作文の自動振り分けルールを再適用します（個別の手動設定は上書きされます）</div>
            <button onClick={reapplyUsageRules} style={{ width:"100%", background:C.card, border:`1px solid ${C.purple}33`, borderRadius:10, padding:"10px 8px", cursor:"pointer", textAlign:"left" }}>
              <div style={{ fontSize:12, fontWeight:700, color:C.purple }}>用途タグを再判定する（{phrases.length}件）</div>
              <div style={{ fontSize:10, color:C.subtle, marginTop:2 }}>長さがあれば英作文、なければクイズ・練習は常にON</div>
            </button>
          </div>

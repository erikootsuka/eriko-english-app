...
- 初級: Short phrases, common everyday words, junior high school English level, simple greetings or basic business expressions
- 中級: Contains multiple clauses, business-specific vocabulary, moderately technical terms, compound sentences
- 上級: Long complex sentences, regulatory/pharmaceutical/legal terminology, complex grammar structures
Return ONLY valid JSON array with no other text: [{"english":"...","level":"初級"|"中級"|"上級"}]`;
  ...
  async function addPhraseWithLevel() {
    ...
    try {
      const sys = `Return ONLY one word — 初級, 中級, or 上級 — difficulty for a Japanese pharmaceutical professional.`;
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      {showAdd && (
        <Modal onClose={() => setShowAdd(false)}>
          ...
          {[{k:"japanese",l:"日本語",ph:"日本語訳"},{k:"english",l:"英語 *",ph:"English phrase"},{k:"context",l:"使う場面",ph:"例: メールの締め"}].map(f => (
            <Field key={f.k} label={f.l}><input value={newP[f.k]} onChange={e=>setNewP(p=>({...p,[f.k]:e.target.value}))} placeholder={f.ph} style={inp} /></Field>
          ))}
          <Field label="カテゴリー">
          ...
          <Field label="難易度（AIが自動判定）">
            <div style={{ display:"flex", gap:6 }}>
              {LEVELS.map(l=>(<button key={l} onClick={()=>setNewP(p=>({...p,level:l}))} style={{ flex:1, padding:"6px 0", borderRadius:8, border:"none", cursor:"pointer", fontSize:12, fontWeight:700, background:newP.level===l?levelColor(l):C.surface, color:newP.level===l?"#fff":levelColor(l) }}>{l}</button>))}
            </div>
          </Field>
      ...
      {showImport && (
        <Modal onClose={() => { setShowImport(false); setImportText(""); setImportResult(null); setImportFiles([]); setImportProgress(""); }}>
          ...
          <p style={{ fontSize:11, color:C.muted, margin:"0 0 12px" }}>ファイルを選択またはテキストを貼り付けてください。</p>
          {!importResult ? (
            <>
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      {showAdd && (
        <Modal onClose={()=>setShowAdd(false)}>
          ...
          {[{k:"word",l:"英語 *",ph:"word / idiom"},{k:"meaning",l:"日本語の意味",ph:"意味"},{k:"example",l:"例文",ph:"Example sentence"}].map(f=>(<Field key={f.k} label={f.l}><input value={newV[f.k]} onChange={e=>setNewV(v=>({...v,[f.k]:e.target.value}))} placeholder={f.ph} style={inp} /></Field>))}
...
- newPhrases: 3-5 useful phrases from the corrected text, level must be one of: 初級, 中級, 上級

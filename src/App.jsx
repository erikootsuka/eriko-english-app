...
// ===================== FIX: HomeTab - single definition only =====================
...
Return ONLY valid JSON array with no other text: [{"english":"...","level":"初級"|"中級"|"上級"}]`;
        ...
        if (needsTranslation.length > 0) {
          ...
          try { const transResp = await callClaude(transSys, JSON.stringify(needsTranslation.map(p => p.english))); const transMatch = transResp.match(/\[[\s\S]*\]/); if (transMatch) translations = JSON.parse(transMatch[0]); } catch {}
        }
        raw.forEach(p => { const foundLevel = leveled.find(l => l.english === p.english); const foundTrans = translations.find(t => t.english === p.english); allPhrases.push({ ...p, level: foundLevel?.level || "中級", japanese: p.japanese || foundTrans?.japanese || "", context: p.context || foundTrans?.context || "", source: src.name }); });
      } catch { raw.forEach(p => allPhrases.push({ ...p, source: src.name })); }
    }
    const seen = new Set();
    const deduped = allPhrases.filter(p => { if (seen.has(p.english)) return false; seen.add(p.english); return true; });
    setImportProgress(""); setImportResult(deduped); setImportLoading(false);
  }
          ...
          {!importResult ? (
            ...
            <>
              ...
              <div style={{ maxHeight:200, overflowY:"auto" }}>
                {importResult.map(p => (<div key={p.id} style={{ padding:"6px 0", borderBottom:`1px solid ${C.surface}`, fontSize:12 }}><div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:2 }}><span style={{ fontSize:9, padding:"1px 6px", borderRadius:99, background:levelBg(p.level), color:levelColor(p.level), fontWeight:700 }}>{p.level}</span></div><span style={{ fontWeight:600 }}>{p.english}</span>{p.japanese && <div style={{ color:C.muted, fontSize:11 }}>{p.japanese}</div>}</div>))}
              </div>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"0 16px 16px" }}>
        {entries.length === 0 && (<div style={{ textAlign:"center", color:C.subtle, padding:"40px 0" }}><div style={{ fontSize:40, marginBottom:12 }}>📔</div><p style={{ fontSize:14 }}>日記をまだ書いていません。<br/>今日の出来事を英語で書いてみましょう！</p></div>)}
        {entries.map(e => (
...
// ===================== HELPERS =====================
// FIX: Single definition of parseCopilotText (removed duplicate)
...
// FIX: RoleplayTab - properly closed function (removed extra ); } at end)

...
// ===================== USAGE TAGS (表現集の用途タグ) =====================
// 表現集の各項目が「クイズ」「練習（シャドーイング/ディクテーション）」「英作文・英訳」
// のどこで出題対象になるかを示す軸。レベル・カテゴリーとは独立した別軸として持たせる。
const USAGE_OPTIONS = [
  { id: "quiz", label: "✏️ クイズ" },
  { id: "practice", label: "🎤 練習" },
  { id: "writing", label: "✍️ 英作文・英訳" },
];
const DEFAULT_USAGES = ["quiz", "practice"]; // 既存データ・新規追加時のデフォルト（英作文は明示的にONにする運用）

// 「英作文・英訳」向けと判定する長さのしきい値。
// 英語・日本語のどちらかがこの文字数以上であれば「ある程度長さがある」とみなす。
// 短い相槨・あいさつ表現は英作文練習としては手応えがなく、シャドーイング/ディクテーション
// （practice）やクイズ（quiz）の方が向いているため対象外とする。
...
// 文字数しきい値にもとづいて用途タグを自動算出する。
// ルール:
// - practice（シャドーイング・ディクテーション）はレベルを問わず常にON（聞く・話すを優先的に鍛えたいため）
// - 長さがあればwriting ON・quiz OFF。長さがなければwriting OFF・quiz ON（quizとwritingは排他）
...
// 表現オブジェクトのusagesを正規化する。usages未設定（旧データ）の場合は自動算出で補う。
...
function PhraseEditModal({ phrase, onSave, onClose, categories }) {
  ...
  // クイズと英作文・英訳（writing）は排他にする。
  // どちらかをONにしたら、もう片方は自動でOFFにする（practiceは常に独立して操作可）。
...
Return ONLY valid JSON array with no other text: [{"english":"...","level":"初級"|"中級"|"上級"}]`;
        ...
        raw.forEach(p => {
          ...
          allPhrases.push({
            ...
            context,
            source: src.name,
            // 文字数しきい値で用途タグを自動算出する（クイズ/英作文は排他、練習は常にON）
  ...
  async function addPhraseWithLevel() {
    ...
    // 文字数しきい値で用途タグを自動算出する（ユーザーが追加フォームで明示的に変更していた場合はそれを尊重）
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      {viewMode === "flash" ? (
        <div style={{ flex:1, padding:"16px", display:"flex", flexDirection:"column", alignItems:"center", overflowY:"auto" }}>
          ...
          {filtered.length === 0 ? <div style={{ color:C.subtle, marginTop:40 }}>表現がありません</div> : (() => {
            const card = filtered[flashIdx % filtered.length];
            const isEn2ja = cardDirection === "en2ja";
            return (
              <>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                  <div style={{ fontSize:11, color:C.subtle }}>{(flashIdx % filtered.length) + 1} / {filtered.length}</div>
                  {direction === "random" && (
...
function QuizTab({ phrases, vocab, setProgress, weaknesses, setWeaknesses }) {
  ...
  // 表現集はさらに「用途」に"クイズ"が含まれる項目のみを対象とする
...
function PracticeTab({ phrases }) {
  ...
  // 表現集の中から「用途」に"英作文・英訳"が含まれる項目を原文として抽出する
  ...
  // さらに「用途」に"練習"が含まれる項目のみを対象とする（聞く・話すの強化を優先するため、
  // レベルを問わず原則すべての表現がpractice対象になるようcomputeAutoUsagesで保証している）
  ...
  // シャドーイング用のレベル絞り込み（カテゴリー絞り込みの結果からさらにレベルで絞る）
